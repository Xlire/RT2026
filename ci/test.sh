...
pathFilters:+**/Assets/Scripts/**,-**/Assets/Scripts/Editor/**;\
assemblyFilters:-EditModeTests,-PlayModeTests" \
...
if grep $CODE_COVERAGE_PACKAGE $PACKAGE_MANIFEST_PATH; then
  ...
  mv $UNITY_DIR/$TEST_PLATFORM-coverage/$CI_PROJECT_NAME-opencov/*Mode/TestCoverageResults_*.xml $UNITY_DIR/$TEST_PLATFORM-coverage/coverage.xml && rm -r $UNITY_DIR/$TEST_PLATFORM-coverage/$CI_PROJECT_NAME-opencov/ || echo "More than 1 coverage result?"
 else
  {
